...
from analytics.enhanced_activity_classifier import classify_activity, compute_activity_confidence
...
def test_posture_robustness():
 ...
 print(f" Returned '{posture}' instead of 'unknown'")
 ...
 print(f" Returned '{bed_angle.get('orientation')}' instead of 'unknown'")
 ...
 print(f" Final state: {results[-1]} (expected 'side')")
 ...
 print(f" Symmetry index: {symmetry.get('symmetry_index')} (expected >0.9)")
...
def test_activity_robustness():
 ...
 print(f" Returned '{activity.get('activity')}' instead of 'unknown'")
 ...
 print(f" Did not detect walking (detected: {activity_walking.get('activity')})")
...
def test_tracking_robustness():
 ...
 print(" Track ID consistency depends on ByteTrack/StrongSORT")
 print(" Need to test with actual video to verify consistency")
 recommendations.append({
 "priority": "HIGH",
 ...
 print(" Current: Uses largest bbox or face recognition")
 print(" Risk: May switch to visitor if face recognition fails")
 ...
 print(" Current threshold: 150 frames (10 seconds at 15 FPS)")
 print(" May be too short for long occlusions (e.g., nurse blocking view)")
 ...
 print(" No validation of track ID consistency across frames")
 recommendations.append({
 "priority": "MEDIUM",
 ...
 print(" Patient onboarding logic not clearly visible in code")
 print(" Need explicit patient onboarding with track_id assignment")
 recommendations.append({
 "priority": "HIGH",
...
def test_integration_robustness():
 ...
 print(f" Validated {len(validated)} keypoints (expected fewer)")
 ...
 print(" One or both returned 'unknown' (may be acceptable)")
 else:
 issues.append("Posture and activity classifications may be inconsistent")
 print(f" Inconsistency: posture={posture_lying}, activity={activity_lying.get('activity')}")
