...
def test_temporal_model():
    ...
    try:
        ...
        label, conf, probs, uncertainty = model.predict(feat_window)

        if (label in model.labels or label == "untrained") and len(probs) == len(model.labels):
            log.info("✅ Temporal Model: PASSED")
            log.info(f"   Label: {label}, Confidence: {conf:.2f}, Uncertainty: {uncertainty:.2f}")
            return True
        else:
